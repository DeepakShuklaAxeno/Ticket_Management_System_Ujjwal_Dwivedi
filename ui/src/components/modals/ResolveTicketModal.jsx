...
export default function ResolveTicketModal({ isOpen, onClose, ticket, onResolved }) {
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-50 dark:bg-slate-800">
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
              Admin review: accept completed work and close
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-semibold text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-600 resize-none"
              autoFocus
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 rounded-xl shadow"
            >
              {loading ? 'Resolving...' : 'Accept & Resolve'}
            </button>
          </div>
        </form>
      </div>
