...
export default function CompleteTicketModal({ isOpen, onClose, ticket, onCompleted }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const updated = await api.completeTicket(ticket.id, {
        message: message.trim() || undefined
      });
      toast.success('Work marked as completed and sent for Admin review');
      setMessage('');
      onCompleted?.(updated);
      onClose();
    } catch (err) {
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-slate-50 dark:bg-slate-800">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900 dark:text-white">
              Complete Work #{ticket.ticket_number}
            </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-semibold text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-600 resize-none"
              autoFocus
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 disabled:opacity-50 rounded-xl shadow"
            >
              {loading ? 'Submitting...' : 'Submit as Done'}
            </button>
          </div>
        </form>
      </div>
